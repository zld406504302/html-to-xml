...
object XmlExtractor {
  ...
  private def mapHtmlEntity(entity: String, code: Int) {
    entityMappings.put(entity.toUpperCase, code)
  }

  /** These elements are always empty elements. End-tag is redundant for these. */
  private val emptyElements = List("br", "meta", "link", "base", "input", "img", "area")

  /** These elements appear only once in an HTML document. */
  private val singleElements = List("html", "head", "body")

  /** These entities are the only entities supported by default in XML. */
  private val xmlEntities = List("amp", "lt", "gt", "quot", "apos")

  /** Mapping entity names to decimal values, to be used in converting entities not supported in XML. */
  private val entityMappings = new util.Hashtable[String, Int]
  mapHtmlEntities()

  /**
   * Convert HTML input to valid XML.
   * @param html HTML text.
   * @return Valid XML.
   */
  def toXml(html: String) = {
    val result = new StringBuilder

    // Standard XML file header, including entities that are likely to be used.
    result.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n")

    val reader = new ParseReader(html)
    val parser = new TagParser(reader)
    val nestingStack = new util.Stack[String]
    var token = parser.getNextToken
    
    // Ignore leading white-space.
    while ((token.isInstanceOf[SpacesToken] || token.isInstanceOf[NewlineToken] || token.isInstanceOf[DoctypeToken])
        && !token.isInstanceOf[EOFToken]) { token = parser.getNextToken }
      ...
      token match {
        case t: TagToken => {
          if (!t.getTag.isEndTag) {
            ...
            val startTag = t.getTag
            if (startTag.isInstanceOf[EmptyElement]) {
              ...
              val startTagName = startTag.getName.toLowerCase
            ...
            val endTagName = endTag.getName.substring(1).toLowerCase
            ...
            if (emptyElements.contains(endTagName)) {
              ...
              def peekNextNested() = {
                if (nestingStack.isEmpty) null
                else nestingStack.peek()
              }

              // Keep element tags matched appropriately.
              var peek = peekNextNested()
              if (peek == null) {
                ...
                if (peek == endTagName) {
                  ...
                  if (singleElements.contains(endTagName)) {
                    ...
                    while (peek != endTagName) {
                      val endtag = (new StringBuilder).append("</").append(peek).append('>')
                    ...
                    val startTag = (new StringBuilder).append("<").append(endTagName).append('>')
        ...
        case t: WordToken => result.append(t.getWord)
        case t: SpacesToken => result.append(t.getSpaces)
        case t: NumberToken => result.append(t.getNumber.toString)
        case t: EntityReferenceToken => result.append(xmlEntity(t.getName))
        case t: PunctuationToken => result.append(t.getCharacter)
        case t: CharacterEntityToken => result.append(t.getCharacter)
        case t: NewlineToken => result.append('\n')
        case t: ScriptToken => {
          if (t.getScript.length() > 0) { // Script element contents are often empty.
            ...
            val script = t.getScript.replaceAll("<\\!\\[CDATA\\[", "").replaceAll("\\]\\]>", "")
            result.append("/*<![CDATA[*/").append(script).append("/*]]>*/")
          }
        }
        case t: CDataToken => result.append("<![CDATA[").append(t.getData).append("]]>")
        case t: CommentToken => result.append("<!--").append(t.getComment).append("-->")
        case t: DoctypeToken => // Ignore.
        case t: ProcessingInstructionToken => // Ignore
        case _ => log.warn("Unexpected token! {}", token.toString)
      }
  ...
  private def xmlEntity(entityName: String) = {
    val entityLowerCase = entityName.toLowerCase
    if (xmlEntities.contains(entityLowerCase)) {
      (new StringBuilder)
        .append('&').append(entityLowerCase).append(';')
        .toString()
    } else {
      val entityUpperCase = entityName.toUpperCase
